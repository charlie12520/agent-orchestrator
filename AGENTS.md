...
- The daemon's **primary (loopback) listener** stays bound to `127.0.0.1`. By default it remains unauthenticated. The only exception is the hidden SuperOrch-managed daemon mode, which may require a memory-only bearer credential plus daemon generation on the primary listener after a valid stdin bootstrap. Do not widen the bind host, persist the credential, or expose that managed credential path on the LAN/mobile listener.
